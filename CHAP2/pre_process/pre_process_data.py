...
def generate_pre_processed_data(gt3x_30Hz_csv_dir_root, valid_days_file, label_map, sleep_logs_file=None, wear_logs_file=None, non_wear_times_file=None, activpal_events_csv_dir_root=None,
    ...
    :param n_start_ID: (Optional) The index of the starting character of the ID in gt3x file names. Indexing starts with 1.
                        If specified `n_end_ID` should also be specified. If both `n_start_ID` and `expression_after_ID` are
    ...
    if valid_days_file is None:
        ...
        with open(valid_days_file) as f:
            ...
            if header[0].strip().lower() != "id" or (header[1].strip() != "date.valid.day" and header[1].strip() != "valid_days"):
                raise Exception(
                    'valid_days_file should have two header columns (ID, Date.valid.day / valid_days).')
            ...
            if len(header) == 5:
                ...
                for line in lines[1:]:
                    ...
                    if start_time is None or end_time is None:
                        raise Exception(
                            "In {}, sleep log times should match one of {}. Found: {}".format(
    ...
    if non_wear_times_file is not None:
        ...
        with open(non_wear_times_file) as f:
            ...
            # CHAP1.0 format
            if len(header) == 5 and header[1].strip() == "date.nw.start":
            ...
            # iWatch format
            elif len(header) >= 8 and header[1].strip() == "wearloc":
                if loc is None:
                    raise Exception('--loc must be provided for iWatch non-wear files. Use "hip" or "wrist".')
            ...
            # SOL format
            elif len(header) == 3 and header[1].strip() == "startnw":
                ...
                raise Exception(
                    "Unsupported non_wear_times_file format. Expected CHAP1.0, iWatch, or SOL headers. Found: {}".format(header)
                )
...
if __name__ == "__main__":
    ...
    optional_arguments.add_argument(
        '--wear-logs-file', help='Path to the wear logs file', required=False)
    optional_arguments.add_argument(
        '--loc', help='Wear location to use when parsing iWatch non-wear files: hip or wrist', choices=["hip", "wrist"], required=False)
    optional_arguments.add_argument(
    ...
    optional_arguments.add_argument('--expression-after-id', help='String or list of strings specifying different string spliting character \
         that should be used to identify the ID from gt3x file name. The first split will be used as the file name', type=str, required=False)
